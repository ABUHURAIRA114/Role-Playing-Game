...
void Scene::ShowPlayerDeathScreen()
{
    Banner* title = dynamic_cast<Banner*>(ui[FindUIObjectIndex("PLA_END_TITLE")]);
    Banner* text  = dynamic_cast<Banner*>(ui[FindUIObjectIndex("PLA_END_TEXT")]);
    if (title) title->_Text()._Text("End Of The Line");
    if (text)  text->_Text()._Text(
        "The warrior fell.\n\n"
        "Not with a cry, but with a silence that spread outward like ripples "
        "\non still water, until the whole village held its breath.\n\n"
        "The Warlord stood over him for a long moment, saying nothing. "
        ...
        "She did not answer. For she had no hope left."
    );
    endScreenVisible = true;
}
...
void GlobalInfo::LoadStatsUI()
{
    float w = SCREEN_WIDTH, h = SCREEN_HEIGHT;

    // Colors consistent with Menu and Pause UI
    Color darkBrown  = (Color){60, 35, 20, 255};
    Color cream      = (Color){245, 235, 210, 255};
    Color btnColor   = (Color){80, 48, 28, 255};
    Color accentGold = (Color){180, 150, 110, 255};
    ...
    scene.AddUIObject(new Banner("STA_BORDER", "",
        (Vector2){panelX - 4, panelY - 4}, (Vector2){panelW + 8, panelH + 8}, 0, darkBrown));
    scene.AddUIObject(new Banner("STA_PANEL", "",
        (Vector2){panelX, panelY}, (Vector2){panelW, panelH}, 0, cream));

    // 3. Header & Total Points
    scene.AddUIObject(new Banner("STA_TITLE", "CHARACTER ATTRIBUTES",
        (Vector2){panelX + 20, panelY + 20}, (Vector2){panelW - 40, 40}, 24, darkBrown));
    
    scene.AddUIObject(new Banner("STA_POINTS_LABEL", "Available Points:",
        (Vector2){panelX + 50, panelY + 75}, (Vector2){200, 30}, 18, accentGold));
    
    // This Banner represents the dynamic point counter
    scene.AddUIObject(new Banner("STA_TOTAL_POINTS", "10", 
        (Vector2){panelX + panelW - 100, panelY + 75}, (Vector2){50, 30}, 22, darkBrown));

    // 4. Stat Rows (Charisma, Strength, Armour)
    string stats[] = {"Charisma", "Strength", "Armour"};
    string ids[]   = {"CHA", "STR", "ARM"};
    
    float startY = panelY + 140;
    float rowGap = 80;
    float labelW = 140;
    float btnSize = 40;

    for(int i = 0; i < 3; i++) {
        float currentY = startY + (i * rowGap);

        // Stat Name
        scene.AddUIObject(new Banner("STA_LBL_"+ids[i], stats[i],
            (Vector2){panelX + 40, currentY}, (Vector2){labelW, btnSize}, 20, darkBrown));

        // Minus Button
        scene.AddUIObject(new Button("STA_BTN_MINUS_"+ids[i], "-",
            (Vector2){panelX + 200, currentY}, (Vector2){btnSize, btnSize}, 20, btnColor));

        // Current Stat Value
        scene.AddUIObject(new Banner("STA_VAL_"+ids[i], "0",
            (Vector2){panelX + 250, currentY}, (Vector2){50, btnSize}, 20, darkBrown));

        // Plus Button
        scene.AddUIObject(new Button("STA_BTN_PLUS_"+ids[i], "+",
            (Vector2){panelX + 310, currentY}, (Vector2){btnSize, btnSize}, 20, btnColor));
    }
...
void GlobalInfo::LoadEndScreenUI()
{
    ...
    scene.AddUIObject(new Banner("PLA_END_OVERLAY", "",
        (Vector2){0, 0}, (Vector2){w, h}, 0, cream));

    scene.AddUIObject(new Banner("PLA_END_TITLE", "",
        (Vector2){w*0.1f, h*0.08f}, (Vector2){w*0.8f, 60}, 38, darkBrown));

    scene.AddUIObject(new Banner("PLA_END_TEXT", "",
        (Vector2){w*0.1f, h*0.22f}, (Vector2){w*0.8f, h*0.55f}, 22, darkBrown));

    scene.AddUIObject(new Banner("PLA_END_HINT", "[Press any key to continue]",
...
void GlobalInfo::PlayerInfo()
{
    ...
    scene.AddUIObject(new Banner("PLA_PL_INV", "", Vector2Zero(), (Vector2){buttonSize, buttonSize}, 50, DARKBROWN));
    scene.AddUIObject(new Banner("PLA_PL_INV_1", "", Vector2Zero(), (Vector2){buttonSize, buttonSize}, 50, DARKBROWN));

    grid.AddElement(scene.ui[scene.FindUIObjectIndex("PLA_PL_INV")]);
    grid.AddElement(scene.ui[scene.FindUIObjectIndex("PLA_PL_INV_1")]);
...
void GlobalInfo::LoadDialogueBox()
{
    ...
    // --- BORDER (BACKGROUND) ---
    scene.AddUIObject(new Banner("PLA_DIALOGUE_BG", "", (Vector2){posX, posY}, (Vector2){boxWidth, boxHeight}, 0, borderColor));
    // --- INNER PANEL ---
    scene.AddUIObject(new Banner("PLA_DIALOGUE_PANEL", "", (Vector2){posX + padding, posY + padding}, (Vector2){boxWidth - 2*padding, boxHeight - 2*padding}, 0, panelColor));
    // --- SPEAKER NAME ---
    scene.dialogueSpeakerTextIdx = scene.uiCount;
    scene.AddUIObject(new Banner("PLA_DIALOGUE_NAME", "Speaker", (Vector2){posX + padding + 10, posY + padding + 5}, (Vector2){300, 40}, 23, borderColor));
    // --- DIALOGUE TEXT ---
    scene.dialogueTextIdx = scene.uiCount;
    scene.AddUIObject(new Banner("PLA_DIALOGUE_TEXT", "Dialogue goes here...", (Vector2){posX + padding + 10, posY + padding + 50}, (Vector2){boxWidth - 40, boxHeight-70}, 25, DARKBROWN));
    ...
    // --- BORDER ---
    scene.AddUIObject(new Banner("PLA_CHOICE_BG", "", (Vector2){posX, posY}, (Vector2){boxWidth, boxHeight}, 0, borderColor));
    // --- INNER PANEL ---
    scene.AddUIObject(new Banner("PLA_CHOICE_PANEL", "", (Vector2){posX + padding, posY + padding}, (Vector2){boxWidth - 2*padding, boxHeight - 2*padding}, 0, panelColor));

    // --- CHOICES TEXT (3 OPTIONS) ---
    scene.choice1Idx = scene.uiCount;
    scene.AddUIObject(new Banner("PLA_CHOICE_1", "Choice 1", (Vector2){posX + padding + 10, posY + padding + 10}, (Vector2){boxWidth - 30, 35}, 20, DARKBROWN));
    scene.AddUIObject(new Banner("PLA_CHOICE_2", "Choice 2", (Vector2){posX + padding + 10, posY + padding + 50}, (Vector2){boxWidth - 30, 35}, 20, DARKBROWN));
    scene.AddUIObject(new Banner("PLA_CHOICE_3", "Choice 3", (Vector2){posX + padding + 10, posY + padding + 90}, (Vector2){boxWidth - 30, 35}, 20, DARKBROWN));
}
...
void GlobalInfo::SetPlayerStats()
{
    Banner* points = dynamic_cast<Banner*>(scene.ui[scene.FindUIObjectIndex("STA_TOTAL_POINTS")]);
    if (points) points->_Text()._Text(to_string(totalStatPoints));
    ...
    RectTransform* rT_Plus[3] = {
        scene.ui[scene.FindUIObjectIndex("STA_BTN_PLUS_CHA")], 
        scene.ui[scene.FindUIObjectIndex("STA_BTN_PLUS_ARM")], 
        scene.ui[scene.FindUIObjectIndex("STA_BTN_PLUS_STR")]
    };
    RectTransform* rT_Minus[3] = {
        scene.ui[scene.FindUIObjectIndex("STA_BTN_MINUS_CHA")], 
        scene.ui[scene.FindUIObjectIndex("STA_BTN_MINUS_ARM")], 
        scene.ui[scene.FindUIObjectIndex("STA_BTN_MINUS_STR")]
    };
    RectTransform* rT_Points[3] = {
        scene.ui[scene.FindUIObjectIndex("STA_VAL_CHA")], 
        scene.ui[scene.FindUIObjectIndex("STA_VAL_ARM")], 
        scene.ui[scene.FindUIObjectIndex("STA_VAL_STR")]
    };

    int* plPoints[3] = {
        &pl->charisma, &pl->armour, &pl->strength
    };
    
    for(int i = 0; i < 3; i++) {
        
        button = dynamic_cast<Button*>(rT_Plus[i]);
        if (button && button->IsClicked()) 
        {
            if (totalStatPoints>0)
            {
                plPoints[i][0] = plPoints[i][0]+1;
                totalStatPoints--;
            }
        }
        button = dynamic_cast<Button*>(rT_Minus[i]);
        if (button && button->IsClicked())
        {
            if (totalStatPoints<10 && plPoints[i][0]>0)
            {
                totalStatPoints++;
                plPoints[i][0] = plPoints[i][0]-1;
            }
        }
        points = dynamic_cast<Banner*>(rT_Points[i]);
